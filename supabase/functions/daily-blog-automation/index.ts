...
// gemini-2.0-flash-preview-image-generation was removed from v1beta API
...
async function generateImage(
  ...
  const prompt = `Create a professional educational illustration for a blog post about '${keyword}' for Indian MBA/CAT exam aspirants. Modern, clean, inspiring style. Show study books, IIM campus buildings, or students studying. Use warm orange and blue tones. No text or words in the image.`;
  ...
  for (const model of IMAGE_GEN_MODELS) {
    try {
      ...
      if (!res.ok) {
        console.warn(`[image-gen] ${model} HTTP ${res.status}: ${await res.text()}`);
        continue;
      }
      ...
      if (!b64) {
        console.warn(`[image-gen] ${model} no image data. Response: ${JSON.stringify(json).slice(0, 300)}`);
        continue;
      }
      ...
      console.log(`[image-gen] ${model} success: ${pub?.publicUrl}`);
      return pub?.publicUrl ?? null;
    } catch (err) {
      console.warn(`[image-gen] ${model} error:`, err);
    }
  }
  console.warn("[image-gen] All models failed");
  return null;
}
