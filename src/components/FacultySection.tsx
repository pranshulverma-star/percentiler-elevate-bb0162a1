      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          {faculty.map((f, i) => (
            <div
              key={f.name}
              className={`text-center space-y-3 group bg-card rounded-2xl p-5 border border-border/50 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 fade-in-up ${gridInView ? "in-view" : ""}`}
            ...
            >
              <div className="relative mx-auto w-fit">
                <Avatar className="h-20 w-20 md:h-24 md:w-24 ring-4 ring-transparent group-hover:ring-primary/50 transition-all duration-300">
                  {f.photo ? (
                    <AvatarImage src={f.photo} alt={`${f.name} – ${f.role} at Percentilers CAT coaching`} width={96} height={96} className="object-cover object-[center_20%]" loading="lazy" />
                  ) : null}
