...
const tools = [
  ...
  { icon: FlaskConical, name: "Practice Lab", benefit: "Solve topic-wise CAT questions with instant scoring and detailed performance analytics.", href: "/practice-lab", isLink: true },
  ...
  return (
    <section id="tools" className="py-10 md:py-16 bg-secondary/50">
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-5 max-w-7xl mx-auto">
          {tools.map((t, i) => (
            <motion.div
              key={t.name}
            ...
            >
              <Card className="p-6 text-center h-full flex flex-col justify-between hover:shadow-lg transition-all duration-300 group relative overflow-hidden">
                ...
                <Button variant="outline" size="sm" className="group-hover:bg-primary group-hover:text-primary-foreground group-hover:border-primary transition-colors duration-300" asChild>
                    <a href={t.href}>
                      {t.name === "Free Courses" ? "Explore Free Courses" : t.name === "My Dashboard" ? "Go to Dashboard" : t.name === "CAT Flashcards" ? "Start Practicing" : t.name === "Practice Lab" ? "Start Solving" : "Use Free Tool"} <ArrowRight className="ml-1 h-3.5 w-3.5" />
                    </a>
                  </Button>
              </Card>
            </motion.div>
