...
import {
  ...
  Sparkles, TrendingUp, Award, Lock, Gift,
} from "lucide-react";
  ...
  return (
    <>
      ...
      <main className="pt-20">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
              <AnimatePresence mode="popLayout">
                {filtered.map((pack, i) => {
                  ...
                  const isHovered = hoveredCard === pack.id;
                  ...
                  return (
                    ...
                    >
                      <Card className={`relative overflow-hidden border-2 ${pack.borderColor} transition-all duration-300 group h-full flex flex-col ${pack.featured ? "ring-2 ring-primary/20" : ""}`}>
                        {/* Gradient header */}
                        <div className={`bg-gradient-to-br ${pack.gradient} p-5 pb-12 relative`}>
                          {pack.featured && (
                          ...
                          <div className="flex items-start justify-between">
                            <div className={`w-14 h-14 rounded-2xl ${pack.iconBg} flex items-center justify-center`}>
                              <Icon className={`h-7 w-7 ${pack.iconColor}`} />
                            </div>
                            <Badge className={`${pack.tagColor} text-[10px]`}>{pack.tag}</Badge>
                          </div>
                          <h3 className="text-lg font-bold text-foreground mt-3">{pack.name}</h3>
                          {pack.subtitle && (
                        ...
                        <div className="relative -mt-6 mx-4">
                          <div className="bg-card border border-border rounded-xl p-3 shadow-sm flex items-center justify-between">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-xs font-bold text-foreground">{pack.xpReward} XP</p>
                                <p className="text-[9px] text-muted-foreground">Completion reward</p>
                              </div>
                            </div>
                            <Badge variant="outline" className="text-[9px] gap-1">
                              <Award className="h-2.5 w-2.5" /> {pack.badge}
                            </Badge>
                          </div>
                        </div>
                        ...
                        <div className="p-5 pt-4 flex-1 flex flex-col">
                          ...
                          <div className="mb-4">
                            ...
                            <p className="text-[9px] text-muted-foreground mt-1">Start practicing to track progress</p>
                          </div>
                            ...
                            <div>
                              {pack.price === 0 ? (
                                ...
                                <div>
                                  ...
                                  {pack.originalPrice && (
                                    <span className="text-xs text-muted-foreground line-through ml-1.5">₹{pack.originalPrice}</span>
                                  )}
                                </div>
                              )}
                            </div>
                ...
                <Card className="p-6 border-2 border-border h-full">
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Top performers this month</p>
                    </div>
                  </div>
                ...
                <Card className="p-6 border-2 border-border h-full">
                  <div className="flex items-center gap-2 mb-5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Complete packs to unlock</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    {testPacks.map((pack, idx) => (
                      <motion.div
                        key={pack.id}
                        className="flex flex-col items-center gap-2 p-3 rounded-xl bg-secondary/40 border border-border group cursor-default"
                      ...
                      >
                        <div className={`w-12 h-12 rounded-2xl ${pack.iconBg} flex items-center justify-center relative`}>
                          <pack.icon className={`h-6 w-6 ${pack.iconColor}`} />
                          <Lock className="absolute -bottom-1 -right-1 h-3.5 w-3.5 text-muted-foreground bg-card rounded-full p-0.5" />
                        </div>
                        <p className="text-[10px] font-semibold text-foreground text-center leading-tight">{pack.badge}</p>
                        <p className="text-[8px] text-muted-foreground">{pack.xpReward} XP</p>
                      </motion.div>
                    ))}
                    {/* Placeholder locked badge */}
                    <motion.div
                      className="flex flex-col items-center gap-2 p-3 rounded-xl bg-secondary/20 border border-dashed border-border opacity-50"
                      initial={{ opacity: 0 }}
                    ...
                    >
                      ...
                      <p className="text-[10px] font-semibold text-muted-foreground text-center">???</p>
                      <p className="text-[8px] text-muted-foreground">Hidden</p>
                    </motion.div>
                  </div>
                  <p className="text-[10px] text-muted-foreground text-center mt-4">
        ...
        {/* CTA Section */}
        <section className="py-14 md:py-20 bg-gradient-to-b from-secondary/40 to-background">
