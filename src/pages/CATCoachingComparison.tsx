...
// Only hero-critical images eagerly loaded
...
const coachingChoices = [
  {
    question: "Batch size you vibe with?", emoji: "👥", xp: 15,
    options: [
  ...
  {
    question: "Mentorship style?", emoji: "🎯", xp: 20,
    options: [
  ...
  {
    question: "Study plan approach?", emoji: "🧠", xp: 20,
    options: [
  ...
  {
    question: "Doubt resolution speed?", emoji: "⚡", xp: 15,
    options: [
  ...
  {
    question: "Faculty caliber?", emoji: "🏆", xp: 15,
    options: [
  ...
  {
    question: "Mock test analysis?", emoji: "📊", xp: 15,
    options: [
  ...
  return (
    <section className="py-20 md:py-28 relative overflow-hidden bg-foreground">
      ...
      <div className="max-w-4xl mx-auto px-6 relative z-10">
        ...
        {/* XP + Progress HUD */}
        <div className="mb-8 rounded-2xl border border-background/10 bg-background/5 backdrop-blur-xl p-4 md:p-5">
...
/* ─── Journey Timeline (lazy-loaded images) ─── */
function JourneyTimeline() {
  const [images, setImages] = useState<string[]>([]);

  useEffect(() => {
    Promise.all([
      import("@/assets/journey-stage-1.png"),
      import("@/assets/journey-stage-2.png"),
      import("@/assets/journey-stage-3.png"),
      import("@/assets/journey-stage-4.png"),
      import("@/assets/journey-stage-5.png"),
      import("@/assets/journey-stage-6.png"),
    ]).then((mods) => setImages(mods.map((m) => m.default)));
  }, []);

  const stages = [
    { badge: "Day 0", number: "01", title: "Lost & Overwhelmed", desc: "3.3 lakh CAT aspirants. No clear plan. Picking coaching based on ads, not results.", tag: "Where most start", accent: "from-muted to-muted/50" },
    { badge: "Week 1", number: "02", title: "Free Profile Evaluation", desc: "30-min strategy call. We map your strengths, target colleges & build your personal roadmap.", tag: "Percentilers begin here", accent: "from-primary/20 to-primary/5" },
    { badge: "Week 2", number: "03", title: "AI Study Plan Activated", desc: "Personalised daily schedule. Section targets. Adjust weekly. Zero guesswork.", tag: "Smart prep starts", accent: "from-primary/20 to-primary/5" },
    { badge: "Months 2–5", number: "04", title: "Live Classes + 32 Mocks", desc: "Expert IIM alumni faculty. Deep post-mock analytics. Weak area targeting.", tag: "Peak training zone", accent: "from-primary/20 to-primary/5" },
    { badge: "Month 6", number: "05", title: "1-on-1 IIM Mentor", desc: "Weekly check-ins. WAT-PI prep. Strategy pivots. You are never alone in this.", tag: "Final push", accent: "from-primary/20 to-primary/5" },
    { badge: "Result Day", number: "06", title: "IIM Offer Letter", desc: "300+ Percentilers students got IIM calls. Your name could be next.", tag: "Join 300+ IIM converts", accent: "from-green-100 to-green-50" },
  ];

  return (
    <section className="relative py-20 md:py-28 bg-background overflow-hidden content-auto" style={{ containIntrinsicSize: "0 800px" }}>
            ...
            return (
              ...
              >
                <div className={`pl-12 md:pl-0 flex ${isEven ? "md:justify-end" : "md:justify-start"} ${isEven ? "" : "md:[direction:ltr]"}`}>
                  <div className={`relative w-full max-w-sm rounded-3xl bg-gradient-to-br ${stage.accent} border border-border/60 p-5 md:p-6 shadow-xl`}>
                    {images[index] ? (
                      <img src={images[index]} alt={stage.title} className="w-full h-52 md:h-60 object-contain" loading="lazy" />
                    ) : (
                      <div className="w-full h-52 md:h-60 bg-muted/30 rounded-xl animate-pulse" />
                    )}
...
/* ─── WhatsApp Screenshots (lazy images) ─── */
function WhatsAppSection() {
  const [images, setImages] = useState<string[]>([]);

  useEffect(() => {
    Promise.all([
      import("@/assets/whatsapp-1.webp"),
      import("@/assets/whatsapp-2.webp"),
      import("@/assets/whatsapp-3.webp"),
      import("@/assets/whatsapp-4.webp"),
      import("@/assets/whatsapp-5.webp"),
      import("@/assets/whatsapp-6.webp"),
    ]).then((mods) => setImages(mods.map((m) => m.default)));
  }, []);

  return (
    <section className="py-20 md:py-28 bg-background relative content-auto" style={{ containIntrinsicSize: "0 600px" }}>
...
/* ─── Lazy-loaded below-fold student images ─── */
function useStudentImages() {
  const [imgs, setImgs] = useState<Record<string, string>>({});
  useEffect(() => {
    Promise.all([
      import("@/assets/student-aditya.webp"),
      import("@/assets/student-ritik.webp"),
      import("@/assets/student-prakhar.webp"),
      import("@/assets/student-saloni.webp"),
      import("@/assets/student-sattaki.webp"),
    ]).then(([aditya, ritik, prakhar, saloni, sattaki]) => {
      setImgs({
        aditya: aditya.default, ritik: ritik.default,
        prakhar: prakhar.default, saloni: saloni.default, sattaki: sattaki.default,
      });
    });
  }, []);
  return imgs;
...
export default function CATCoachingComparison() {
  ...
  const extraImgs = useStudentImages();
  ...
  const results = [
    ...
    { name: "Aditya Kumar", percentile: "98.6", college: "XLRI", initials: "AK", photo: extraImgs.aditya, quote: "Daily planner kept me disciplined throughout." },
    ...
    { name: "Ritik Kumar", percentile: "98.1", college: "IIM Udaipur", initials: "RK", photo: extraImgs.ritik, quote: "Strategy over hours changed everything." },
    { name: "Prakhar Poddar", percentile: "98.0", college: "IIM Trichy", initials: "PP", photo: extraImgs.prakhar, quote: "Right guidance at the right time." },
    { name: "Saloni Hindocha", percentile: "98.4", college: "IIT Mumbai", initials: "SH", photo: extraImgs.saloni, quote: "Expert feedback was key to my success." },
    { name: "Sattaki Basu", percentile: "98.2", college: "IIM Ranchi", initials: "SB", photo: extraImgs.sattaki, quote: "Mock analysis transformed my approach." },
  ];
  ...
  return (
    <div className="bg-background min-h-screen">
      ...
      {/* ═══ HERO — No animations, instant render ═══ */}
      ...
      {/* Below-fold sections with content-visibility */}
      <IdealCoachingPicker scrollTo={scrollTo} />

      <JourneyTimeline />

      {/* ═══ CHAPTER 1: THE NUMBERS ═══ */}
      <section className="py-20 md:py-28 bg-background relative content-auto" style={{ containIntrinsicSize: "0 400px" }}>
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-4">
            <AnimatedStat value={300} suffix="+" label="IIM Converts" />
            <AnimatedStat value={10000} suffix="+" label="Students Trained" />
            <AnimatedStat value={99} suffix="%" label="Faculty Percentile" />
            <AnimatedStat value={8} suffix=" yrs" label="Coaching Experience" />
          </div>
        </div>
      </section>

      {/* ═══ CHAPTER 2: THE PROBLEM ═══ */}
      <section className="py-20 md:py-28 bg-[hsl(25,100%,97%)] relative content-auto" style={{ containIntrinsicSize: "0 600px" }}>
        <div className="max-w-5xl mx-auto px-6">
      ...
      {/* ═══ DRAMATIC DIVIDER ═══ */}
      <section className="py-16 md:py-24 bg-background relative overflow-hidden content-auto" style={{ containIntrinsicSize: "0 200px" }}>
      ...
      <section className="py-20 md:py-28 bg-background relative content-auto" style={{ containIntrinsicSize: "0 500px" }}>
      ...
      {/* ═══ CTA: COUNSELING CALL ═══ */}
      <section id="call-section" className="py-20 md:py-28 bg-background relative border-t border-border content-auto" style={{ containIntrinsicSize: "0 500px" }}>
      ...
      {/* ═══ FAQ ═══ */}
      <section className="py-20 md:py-28 bg-[hsl(25,100%,97%)] relative content-auto" style={{ containIntrinsicSize: "0 400px" }}>
